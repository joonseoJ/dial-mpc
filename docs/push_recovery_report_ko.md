...
"잘 버텼는지"를 평가하는 항목을 네 가지로 나눈다. 각 항목은 nominal 상태로부터의
편차에 대한 quadratic penalty다.
...
네 개의 basis weight $\{\omega_1, \dots, \omega_4\}$ 를 골라 각각 network를 학습했다고
하자. 사용자가 방향 $\omega^*$ 와 날카로움 $T^*$ 를 요청하면, 필요한 계수 $a$ 는

$$\sum_i a_i\, \nu_i = \nu^* \quad\Longleftrightarrow\quad
\sum_i a_i \frac{\omega_i}{T} = \frac{\omega^*}{T^*}$$

를 푸는 것이고, $B$ 를 basis weight를 행으로 쌓은 행렬이라 하면 pseudoinverse로

$$\boxed{\ a = \frac{T}{T^*}\ \omega^{*\top} B^{+}\ }$$
...
- **basis 자신을 요청하면 그 network 하나만 쓰인다.** $\omega^* = \omega_j$,
  $T^*=T$ 이면 $a = e_j$ 가 정확히 나온다. 단, 이는 $B$ 가 full row rank일 때만
  성립한다 — basis weight의 개수가 항목 수(여기서는 4)를 넘으면 pseudoinverse가
...
같은 세기로 밀었을 때 weight에 따라 회복 전략이 달라진다. 아래는 학습된 컨트롤러의
측정값이다.

**몸통 기울기** (작을수록 몸통이 수평)

| 목표 weight | 0.25 m/s | 0.45 | 0.70 | 0.90 |
|---|---|---|---|---|
| **tilt 강조** | **0.008** | **0.011** | **0.019** | **0.031** |
| base 강조 | 0.025 | 0.029 | 0.049 | 0.081 |
| feet 강조 | 0.017 | 0.029 | 0.052 | **0.089** |
| shape 강조 | 0.019 | 0.024 | 0.043 | 0.073 |
| 균등 | 0.014 | 0.018 | 0.030 | 0.054 |

tilt를 강조하면 균등 대비 43% 낮고, feet 강조 대비 **2.9배** 낮다.

**몸통 중심 이동** (m, 작을수록 제자리)

| 목표 weight | 0.25 m/s | 0.45 | 0.70 | 0.90 |
|---|---|---|---|---|
| tilt 강조 | 0.011 | 0.021 | 0.042 | 0.068 |
| **base 강조** | **0.009** | **0.016** | **0.032** | **0.048** |
| feet 강조 | 0.024 | 0.032 | 0.049 | **0.071** |
| shape 강조 | 0.011 | 0.021 | 0.042 | 0.068 |
| 균등 | 0.010 | 0.018 | 0.036 | 0.059 |

base를 강조하면 중심 이동이 최소가 되지만, 위 표에서 보듯 그 대가로 기울기가 가장
커진다. **§1.1에서 예측한 center of pressure trade-off가 그대로 측정된다.**

**발을 떼는 확률** — 가장 극적으로 갈리는 지표

| 목표 weight | 0.25 m/s | 0.45 | **0.70** | 0.90 |
|---|---|---|---|---|
| tilt 강조 | 0.00 | 0.00 | **1.00** | 1.00 |
| (2,1,1,1) | 0.00 | 0.00 | **0.88** | 1.00 |
| feet 강조 | 0.00 | 0.00 | **0.25** | 1.00 |
| 균등 | 0.00 | 0.00 | **0.13** | 1.00 |
| base 강조 | 0.00 | 0.00 | **0.13** | 1.00 |
| (1,2,2,1) | 0.00 | 0.00 | **0.00** | 1.00 |

0.70 m/s에서 **같은 세기의 외란에 대해 어떤 weight는 항상 발을 떼고 어떤 weight는
절대 떼지 않는다.** 0.25~0.45 m/s에서는 모든 weight가 발을 붙이고 버티며 기울기만
갈리고, 0.90 m/s에서는 모두 발을 뗀다. 즉 **0.6~0.75 m/s 부근이 전략이 갈리는
전이 구간**이다.

**발이 공중에 떠 있던 시간의 비율**

| 목표 weight | 0.45 m/s | 0.70 | 0.90 |
|---|---|---|---|
| base 강조 | 0.062 | 0.238 | **0.588** |
| tilt 강조 | 0.025 | 0.192 | 0.542 |
| shape 강조 | 0.008 | 0.163 | 0.533 |
| **feet 강조** | **0.000** | **0.179** | **0.467** |

feet를 강조하면 발을 가장 적게 뗀다.
...
0.90 m/s 외란. 선명한 쪽이 `(1,0,0,0)`(기울기 최소화만), 반투명 쪽이
`(0,1,0,0)`(위치 유지만)이다. 두 요청 모두 basis 볼록 껍질 밖이지만 거동은 여전히
...
왼쪽부터 tilt / base / feet / shape 강조. 이 세기는 §3.4에서 본 **전략이 갈리는
...
선명한 쪽이 tilt 강조, 반투명한 쪽이 feet 강조다. 이 세기에서 발을 떼는 확률은
tilt 강조가 1.00, feet 강조가 0.25다. 영상에서 선명한 로봇만 발을 들어 옮기고
