...
class UnitreeGo2EnvConfig(BaseEnvConfig):
    ...
    # Row normalizers.  One at each means the objective is bit-for-bit the
    # original Go2 trot reward, which is what every earlier run used; the
    # screen measures the rows' spread across a sample cloud and these are
    # where the equalisation goes.
...
class UnitreeGo2Env(BaseEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # Store the normalised weights back, as every other environment here
        # does.  Anything downstream that records "which omega produced this"
        # reads `info`, and leaving one task's copy un-normalised would mean the
        # recorded weight and the weight the reward used were different objects.
